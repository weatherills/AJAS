...
def list_jobs(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        body = get_service().list_feed(
            ...
            location=req.params.get("location") or "",
            status=req.params.get("status") or "all",
...
@bp.route(route="v1/jobs/{id}", methods=["GET"])
def get_job(req: func.HttpRequest) -> func.HttpResponse:
    try:
        _auth(req)
        return json_response(get_service().get_feed_job(req.route_params["id"]))
    except Exception as exc:
        return _handle(exc)


@bp.route(route="v1/sources/status", methods=["GET"])
def list_source_status(req: func.HttpRequest) -> func.HttpResponse:
    try:
        _auth(req)
        return json_response(get_service().source_status())
    except Exception as exc:
        return _handle(exc)
