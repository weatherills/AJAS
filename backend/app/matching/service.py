...
class MatchingService:
    ...
    def _rank_pairs(self, user_id: str, body: dict) -> list[dict]:
        ...
        if len(job_ids) + len(job_texts) > cfg.match_max_rank_pairs:
        ...
        for job_id in job_ids:
            ...
            _id, text = self._resolve_text(user_id, job_id, None, id_path="jobIds", text_path="jobTexts", kind="job")
            pairs.append(
                {
                    "resume_id": resume_id,
                    "resume_text": resume_text,
        ...
        for text in job_texts:
            _id, resolved = self._resolve_text(user_id, None, text, id_path="jobIds", text_path="jobTexts", kind="job")
            pairs.append(
                {
                    "resume_id": resume_id,
                    "resume_text": resume_text,
        ...
        if len(pairs) > cfg.match_max_rank_pairs:
            raise MatchingValidationError(f"at most {cfg.match_max_rank_pairs} pairs per operation", path="jobIds")
        return pairs
