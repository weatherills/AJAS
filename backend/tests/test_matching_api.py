...
def test_high_score_persists_and_low_score_does_not(svc):
    ...
    listed = _body(routes.list_matches(_req("GET", "http://localhost/api/v1/matches")))
...
def test_list_matches_is_user_owned(svc):
    ...
    mine = _body(routes.list_matches(_req("GET", "http://localhost/api/v1/matches")))
    theirs = _body(routes.list_matches(_req("GET", "http://localhost/api/v1/matches", user=OTHER)))
    assert len(mine["items"]) == 1
    assert theirs["items"] == []
    filtered = _body(
        routes.list_matches(_req("GET", "http://localhost/api/v1/matches", params={"jobId": "owned", "minScore": "70"}))
    )
