#!/usr/bin/env python3
"""Create-or-complete a Sprint 11 CodeSpring Kanban card by exact title."""
...
def main() -> int:
    ...
    if not title:
        print("usage: s11_kanban.py '<title>'", file=sys.stderr)
        return 2
    ...
    if match is None:
        created = subprocess.check_output(
            ["codespring", "task", "create", "--title", title, "--priority", "high", "--json"],
            text=True,
        )
