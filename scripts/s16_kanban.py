#!/usr/bin/env python3
"""Create-or-complete a Sprint 16 CodeSpring Kanban card by exact title."""
...
def main() -> int:
    ...
    if not title:
        print("usage: s16_kanban.py '<title>' [sha] [nnn]", file=sys.stderr)
        return 2
    wanted = {title, title.replace("[S16] ", "[Sprint 16] ", 1)}
    if number.isdigit():
        wanted.add(f"[S16] Task {int(number):03d}")
    try:
        tasks = _load()
